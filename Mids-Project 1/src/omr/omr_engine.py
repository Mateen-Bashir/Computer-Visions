...
def _block_score(preds: list[QuestionPrediction], answer_key: dict[int, str], start_q: int, end_q: int) -> tuple[int, float]:
    correct = 0
    confs: list[float] = []
    for p in preds:
        if start_q <= p.question_id <= end_q:
            confs.append(p.confidence)
            if p.answer == answer_key.get(p.question_id, ""):
                correct += 1
    avg_conf = float(np.mean(confs)) if confs else 0.0
    return correct, avg_conf


def _offset_block(points: list[BubblePoint], start_q: int, end_q: int, dx: int, dy: int) -> list[BubblePoint]:
    out: list[BubblePoint] = []
    for p in points:
        if start_q <= p.question_id <= end_q:
            out.append(
...
def _optimize_block_offsets(
    gray: np.ndarray,
    base_points: list[BubblePoint],
    config: OMRConfig,
    answer_key: dict[int, str],
) -> list[BubblePoint]:
    points = base_points
    offsets = [(-12, 0), (12, 0), (0, -12), (0, 12), (-8, -8), (8, 8), (-8, 8), (8, -8), (0, 0)]
    for start_q, end_q in ((1, 10), (11, 20), (21, 30)):
        best_pts = points
        base_preds = _predict_from_points(gray, points, config, cnn_model=None, use_cnn=False)
        best_score = _block_score(base_preds, answer_key, start_q, end_q)
        for dx, dy in offsets:
            cand = _offset_block(points, start_q, end_q, dx, dy)
            preds = _predict_from_points(gray, cand, config, cnn_model=None, use_cnn=False)
            score = _block_score(preds, answer_key, start_q, end_q)
            if score > best_score:
                best_score = score
                best_pts = cand
        points = best_pts
...
def evaluate_sheet(
    ...
    if calibrated_points is not None:
        ...
        if snapped is not None:
            ...
            tuned_blocks = _optimize_block_offsets(gray, snapped, config, answer_key)
            candidates.append(tuned_blocks)
            tuned_snapped = _snap_points_to_detected_circles(gray, tuned_blocks)
            if tuned_snapped is not None:
                candidates.append(tuned_snapped)
                tuned_warped = _local_block_warp_points(gray, tuned_snapped)
                if tuned_warped is not None:
                    candidates.append(tuned_warped)
    for fn in (
    ...
    # Stage-1: fast selection without CNN to avoid expensive inference on all candidates.
    scored_candidates: list[tuple[tuple[int, float], list[BubblePoint]]] = []
    for pts in candidates:
        preds = _predict_from_points(gray, pts, config, cnn_model=None, use_cnn=False)
        summary = compute_score_summary(preds, answer_key)
        score_tuple = (int(summary["correct"]), float(summary["avg_confidence"]))
    ...
    for pts in shortlisted:
        preds = _predict_from_points(gray, pts, config, cnn_model, use_cnn=True)
        summary = compute_score_summary(preds, answer_key)
        score_tuple = (int(summary["correct"]), float(summary["avg_confidence"]))
